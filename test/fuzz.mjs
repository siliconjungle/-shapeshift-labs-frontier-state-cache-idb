...
const seed = readPositiveInt(args.seed, 0x1dbca5);
...
console.log('frontier state-cache-idb fuzz passed cases=' + cases + ' seed=' + seed);

async function runCase(caseId, rng) {
  const indexedDB = createFakeIndexedDB();
  const storage = createQueryCacheIndexedDbStorageAdapter({
    databaseName: 'frontier-idb-fuzz-' + caseId,
    indexedDB,
    now: () => caseId
  });
